...
if git status --porcelain | grep -q .; then
  ...
  if git diff --cached --name-only | grep -q .; then
    # Detect new Markdown posts
    new_posts=$(git status --porcelain "$POSTS_DESTINATION" | grep "^??" | awk '{print $2}')
    if [[ -n "$new_posts" ]]; then
      post_titles=()
      for post in $new_posts; do
        # Extract first Markdown heading (# Title)
        title=$(grep -m 1 '^# ' "$post" | sed 's/^# //')
        # Fallback to filename without extension if no title found
        [[ -z "$title" ]] && title=$(basename "$post" .md)
        post_titles+=("$title")
      done

      # Create a concise commit message
      if [[ ${#post_titles[@]} -le 3 ]]; then
        commit_message="Add new post(s): ${post_titles[*]}"
      else
        commit_message="Add ${#post_titles[@]} new posts, including: ${post_titles[0]}, ${post_titles[1]}, ${post_titles[2]}"
      fi
    else
      commit_message="Update blog on $(date '+%Y-%m-%d %H:%M:%S')"
    fi
